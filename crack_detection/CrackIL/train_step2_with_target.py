...
def train(args, model, model_old, discriminator):
    ...
    target_loader_train = DataLoader(target_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=False)
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        if i < 10:
            ...
            for step, sample in enumerate(target_loader_train):
                ...
                print(outputs_prev_model.shape)
                targets = torch.argmax(outputs_prev_model, dim=1).detach().cuda()
