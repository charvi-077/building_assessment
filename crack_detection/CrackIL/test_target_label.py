...
class CrackTargetNew(Dataset):
    ...
    def __init__(self, args, base_dir, label_dir):

        super().__init__()

        self._base_dir = base_dir
        self._image_dir = os.path.join(self._base_dir, 'images')
        self._label_dir = os.path.join(label_dir, 'labels')
    ...
    def _make_img_gt_point_pair(self, index):
        _img = Image.open(self.images[index]).convert('RGB')
        _img = _img.resize((self.args.crop_size_width, self.args.crop_size_height), Image.BILINEAR)
        _label = Image.open(self.labels[index]).convert('L')
        _label = np.asarray(_label)
        new_label = np.zeros(_label.shape)
        new_label[_label==255] = 1
        _label = Image.fromarray(new_label)
        _label = _label.resize((self.args.crop_size_width, self.args.crop_size_height), Image.NEAREST)
...
def main(args):
    ...
    target_dataset = CrackTargetNew(args = args, base_dir = args.target_dataset_path, label_dir = args.label_dataset_path)
    ...
    eval_model(model, target_loader, args.output_folder, 0, criterion, evaluator)
    
    # for k, v in loaded_model['state_dict'].items():
    #         print(k, v)
    
# -------------- model evaluateion 
def eval_model(model, dataset_loader, output_folder, task, criterion, evaluator):
    model.eval()
    epoch_loss_val = []
    time_val = []
    evaluator.reset()
    ...
    with torch.no_grad():
        for step, sample in enumerate(dataset_loader):
            images, image_ids, image_sizes, labels = sample['image'], sample['image_id'], sample['image_size'], sample['label']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_dataset_path', type=str, default='/home2/kushagra0301/CrackIL/eval_data/crack/', help='label dataset')
    parser.add_argument('--output_folder', type=str, default='/home2/kushagra0301/CrackIL/eval_data/crack/output', help='output eval result of only images')
