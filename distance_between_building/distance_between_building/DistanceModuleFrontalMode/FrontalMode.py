...
class AdjacentRoofDistance:
    ...
    def getBuildings(self,cloud,k,l,a):
        ...
        for i in range(k):
            ...
            result = self.segment_plane_RANSAC(new_cloud)
            if result is None:
            # Handle the case where no plane is found
                print("No plane found.")
                continue
    ...
    def selectPoints(self,cloud,m,yl,ya,offset,index,str):
        ...
        for i in range(m):
            ...
            if mypoints.shape[0] == 0:
                print(f"Warning: Empty point cloud at iteration {i}")
    ...
    def EstimateDistanceAdjacentBuildings(self):
        ...
        for i in range(4):
            if (i >= len(listtrueposR) or i >= len(listtrueposL)):
                print(f"Warning: Insufficient elements in listtrueposR. Skipping index {i}")
                continue
        ...
        distlist=[]
        for i in range(4):
            if 2*i >= len(linepoints) or 2*i+1 >= len(linepoints):
                print(f"Warning: Insufficient elements in linepoints. Skipping index {i}")
                continue
